...
Stm32单片机通常有HAL库（LL库），标准库，寄存器三种开发方式，不同开发方式有着不同的工程结构。HAL库又称为硬件抽象层（ Hardware Abstraction Layer），Hal库为开发者提供了许多函数和模块，是开发者能够通过调用对应函数对外设进行操作，代码的可移植性好，但是由于层层封装性能开销较大；标准库的可移植性较差，意法半导体公司不再提供对标准库的支持。
